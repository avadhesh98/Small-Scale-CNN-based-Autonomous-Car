...
{
  //Read image
  std::string pipeline = "nvarguscamerasrc ! video/x-raw(memory:NVMM), width=1280, height=720, format=NV12, framerate=30/1 ! nvvidconv ! video/x-raw, width=1024, height=768, format=BGRx ! videoconvert ! video/x-raw, format=BGR ! appsink drop=1"
  cv::Mat frame;
